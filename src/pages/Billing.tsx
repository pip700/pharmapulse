...
export const Billing: React.FC<BillingProps> = ({ formatCurrency }) => {
  // Tabs: 'pos' or 'history'
              ...
              {filteredMeds.map(med => (
                ...
                >
                  <div className="flex justify-between items-start">
                    ...
                    <span className="font-bold text-teal-600">{formatCurrency(med.sellingPrice)}</span>
                  </div>
                ...
                cart.map(item => (
                  <div key={item.id} className="flex justify-between items-center bg-gray-50 p-3 rounded-lg">
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-gray-500">{formatCurrency(item.sellingPrice)} x {item.quantity}</p>
                    </div>
                     ...
                     <tbody className="divide-y divide-gray-100">
                         {filteredHistory.length > 0 ? filteredHistory.map(sale => (
                             <tr key={sale.id} className="hover:bg-gray-50 transition-colors">
                                 ...
                                 <td className="p-4 text-sm font-bold text-teal-600 text-right">{formatCurrency(sale.totalAmount)}</td>
